...
count x xs = foldr f 0 xs
  where f y acc = if x == y then acc + 1 else acc
...
inBoth xs ys = foldr f [] xs
  where f x acc = if elem x ys then x : acc else acc
...
sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mempty
sumTree (Node x ls rs) = sumTree ls <> x <> sumTree rs
