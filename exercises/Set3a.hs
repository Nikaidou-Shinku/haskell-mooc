...
maxBy :: (a -> Int) -> a -> a -> a
maxBy measure a b = if resa > resb then a else b
  where
    resa = measure a
    resb = measure b
...
while check update value = head . dropWhile check $ [applyN update x value | x <- [0..]]
...
whileRight check x = takeLeft . head . dropWhile dropCond $ [applyN check' times (Right x) | times <- [0..]]
  where
    check' (Right x) = check x
    check' x = x
    dropCond (Right _) = True
    dropCond _ = False
    takeLeft (Left x) = x
    takeLeft _ = error "WTF?"
...
joinToLength len xs = filter (\x -> length x == len) [x ++ y | x <- xs, y <- xs]
...
interpreter' _ _ [] = []
interpreter' x y (command : others)
  | command == "up" = interpreter' x (y + 1) others
  | command == "down" = interpreter' x (y - 1) others
  | command == "left" = interpreter' (x - 1) y others
  | command == "right" = interpreter' (x + 1) y others
  | command == "printX" = show x : interpreter' x y others
  | command == "printY" = show y : interpreter' x y others
  | otherwise = error "WTF?"
