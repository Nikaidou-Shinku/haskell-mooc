...
allValues condition (Node val left right)
  | not . condition $ val = False
  | not . allValues condition $ left = False
  | not . allValues condition $ right = False
  | otherwise = True
...
cull _ Empty = Empty
cull val (Node val' left right) = if val == val' then Empty else Node val' (cull val left) (cull val right)
...
isOrdered (Node val left right)
  | not . allValues (<= val) $ left = False
  | not . allValues (>= val) $ right = False
  | otherwise = isOrdered left && isOrdered right
...
walk (x : xs) (Node _ left right) = if x == StepL then walk xs left else walk xs right
...
set (x : xs) val (Node val' left right) = if x == StepL then Node val' (set xs val left) right else Node val' left (set xs val right)
...
search val (Node val' left right) = if val == val' then Just [] else
  case (search val left, search val right) of
    (Just x, _) -> Just (StepL : x)
    (_, Just x) -> Just (StepR : x)
    _ -> Nothing
