...
instance Eq a => Eq (List a) where
  ...
  (==) Empty _ = False
  (==) _ Empty = False
  (==) (LNode x xs) (LNode y ys) = if x /= y then False else xs == ys
...
instance (Price a) => Price [a] where
  price xs = foldr f 0 xs
    where f x res = price x + res
...
instance Num RationalNumber where
  ...
  signum (RationalNumber a b) = if a == 0 then RationalNumber 0 1 else RationalNumber sig 1
    where sig = if signum a == signum b then 1 else -1
