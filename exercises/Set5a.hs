...
rgb Green = [0, 1, 0]
rgb Blue = [0, 0, 1]
rgb (Mix col1 col2) = [(r1 + r2) / 2, (g1 + g2) / 2, (b1 + b2) / 2]
  where [r1, g1, b1] = rgb col1
        [r2, g2, b2] = rgb col2
...
fromList xs = foldr f Empty xs
  where f (key, value) res = Pair key value res
...
toNat n
  | n < 0 = Nothing
  | n == 0 = Just Zero
  | otherwise = Just (PlusOne prev)
  where prev = case toNat (n - 1) of
          Nothing -> error "WTF??"
          Just k -> k
