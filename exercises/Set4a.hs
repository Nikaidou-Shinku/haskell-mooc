...
allEqual [] = True
allEqual xs = case allEqual' xs of
  Just _ -> True
  Nothing -> False
...
incrementKey k = map f
  where f (k', v) = if k == k' then (k', v + 1) else (k', v)
...
freqs = foldr f Map.empty
  where f x res = Map.alter g x res
        g Nothing = Just 1
        g (Just x) = Just (x + 1)
...
-- The model solution gives a much more elegant method with pattern matching than mine
-- That's really cooooooooool
transfer :: String -> String -> Int -> Map.Map String Int -> Map.Map String Int
transfer from to amount bank
  | Map.notMember from bank = bank
  | Map.notMember to bank = bank
  | amount < 0 = bank
  | amount > fromAmount = bank
  | otherwise = trans from to amount bank
  where fromAmount = Map.findWithDefault 0 from bank
        toAmount = Map.findWithDefault 0 to bank
        trans from to amount bank = Map.insert to (toAmount + amount) (Map.insert from (fromAmount - amount) bank)
...
maxIndex arr = fst . maxIndex' . Data.Array.assocs $ arr

maxIndex' :: (Ix i, Ord a) => [(i, a)] -> (i, a)
maxIndex' [] = error "empty array"
maxIndex' [x] = x
maxIndex' ((i, x) : xs) = if x > snd res then (i, x) else res
  where res = maxIndex' xs
