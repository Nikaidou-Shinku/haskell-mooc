...
data State = Start | WithEggs | WithFlour | WithSugar | WithBothFlourAndSugar | Mixed | Error | Finished
  deriving (Eq,Show)
...
step WithFlour AddSugar = WithBothFlourAndSugar
step WithSugar AddFlour = WithBothFlourAndSugar
step WithBothFlourAndSugar Mix = Mixed
...
reverseNonEmpty (x :| []) = x :| []
reverseNonEmpty (x :| xs) = head :| (res ++ [x])
  where (head : res) = reverse xs
...
-- This just like the expression problem lol
...
passwordAllowed pswd (MinimumLength n) = length pswd >= n
passwordAllowed pswd (ContainsSome xs) = any (`elem` pswd) xs
passwordAllowed pswd (DoesNotContain xs) = all (\c -> not . elem c $ pswd) xs
passwordAllowed pswd (And a b) = passwordAllowed pswd a && passwordAllowed pswd b
passwordAllowed pswd (Or a b) = passwordAllowed pswd a || passwordAllowed pswd b
...
data Arithmetic = Num Integer | Add Arithmetic Arithmetic | Mul Arithmetic Arithmetic
  deriving Show
